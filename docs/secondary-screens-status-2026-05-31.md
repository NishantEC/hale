...
- **Stress Monitor (§4.3)** — new reusable `HalfArcGauge` (banded arc + needle) + `ContributorList` (Today avg vs 7d, peak, recovery, sleep).
...
Still open in Wave 2: HRV detail (D/W/M/Y + drivers — needs NEW backend multi-range trends + correlations); Stress 24h scrubber + coaching card; Health-tab extra trend cards (Cardio Fitness / Body Battery — need new metrics).
