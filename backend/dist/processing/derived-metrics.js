"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.computeDerivedMetrics = computeDerivedMetrics;
const utils_1 = require("./utils");
const STRAIN_LN_7201 = 8.882_643_961_783_384;
function computeDerivedMetrics(samples, sensorRecords, nightFeatures, sleepDetections, baseline, referenceDate) {
    const dayStart = new Date(referenceDate);
    dayStart.setHours(0, 0, 0, 0);
    const dayEnd = new Date(dayStart.getTime() + 24 * 60 * 60 * 1000);
    const stress = stressPoints(samples);
    const sensorSamples = sensorRecords
        .filter((r) => r.spo2Red != null && r.spo2IR != null && r.skinTempRaw != null)
        .map((r) => ({
        timestamp: r.timestamp,
        spo2Red: r.spo2Red,
        spo2IR: r.spo2IR,
        skinTempRaw: r.skinTempRaw,
    }));
    const spo2 = spo2Points(sensorSamples);
    const skinTemp = skinTemperaturePoints(sensorSamples);
    const daySamples = samples.filter((s) => s.timestamp >= dayStart && s.timestamp < dayEnd);
    const strain = strainScore(daySamples, baseline);
    const stressAvg = averageInDay(stress, dayStart, dayEnd);
    const spo2Avg = averageInDay(spo2, dayStart, dayEnd);
    const skinTempAvg = averageInDay(skinTemp, dayStart, dayEnd);
    const baselineStart = new Date(dayStart.getTime() - 7 * 24 * 60 * 60 * 1000);
    const skinTempBaseline = averageInRange(skinTemp, baselineStart, dayStart);
    const recentStart = new Date(dayStart.getTime() - 6 * 24 * 60 * 60 * 1000);
    const sleepConsistency = sleepConsistencyScore(sleepDetections, referenceDate) ??
        sleepConsistencyScoreFromNightFeatures(nightFeatures, referenceDate);
    let detectedSleepNights;
    if (sleepDetections.length === 0) {
        detectedSleepNights = nightFeatures.filter((f) => f.nightDate >= recentStart &&
            f.nightDate <= referenceDate &&
            f.validCoverage >= 0.35).length;
    }
    else {
        detectedSleepNights = sleepDetections.filter((d) => d.nightDate >= recentStart &&
            d.nightDate <= referenceDate &&
            d.validCoverage >= 0.35).length;
    }
    return {
        stressScores: stress,
        spo2Scores: spo2,
        skinTempScores: skinTemp,
        strainScore: strain,
        sleepConsistencyScore: sleepConsistency,
        detectedSleepNights,
        skinTempAvgCelsius: skinTempAvg,
        skinTempDeltaCelsius: skinTempAvg != null && skinTempBaseline != null
            ? skinTempAvg - skinTempBaseline
            : null,
        stressAverage: stressAvg,
        spo2Average: spo2Avg,
    };
}
function averageInDay(points, dayStart, dayEnd) {
    return averageInRange(points, dayStart, dayEnd);
}
function averageInRange(points, start, end) {
    const filtered = points.filter((p) => p.timestamp >= start && p.timestamp < end);
    if (filtered.length === 0)
        return null;
    return (0, utils_1.average)(filtered.map((p) => p.value));
}
function stressPoints(samples) {
    const sorted = [...samples].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    const windowSize = 120;
    const step = 30;
    if (sorted.length < windowSize)
        return [];
    const output = [];
    for (let start = 0; start <= sorted.length - windowSize; start += step) {
        const window = sorted.slice(start, start + windowSize);
        const rr = [];
        for (const sample of window) {
            if (sample.ibiMs != null) {
                rr.push(sample.ibiMs);
            }
            else if (sample.heartRate > 0) {
                rr.push(60_000.0 / sample.heartRate);
            }
        }
        if (rr.length < windowSize)
            continue;
        const score = baevskyStressScore(rr);
        if (score == null)
            continue;
        const time = window[window.length - 1].timestamp;
        output.push({ timestamp: time, value: score });
    }
    return output;
}
function baevskyStressScore(rrMs) {
    if (rrMs.length < 120)
        return null;
    const clamped = rrMs.map((v) => Math.min(2000, Math.max(250, v)));
    const minRR = Math.min(...clamped);
    const maxRR = Math.max(...clamped);
    const vr = (maxRR - minRR) / 1000;
    if (vr < 0.0001)
        return 10.0;
    const binWidth = 50.0;
    const bins = new Map();
    for (const value of clamped) {
        const bin = Math.floor(value / binWidth);
        bins.set(bin, (bins.get(bin) ?? 0) + 1);
    }
    let modeBin = 0;
    let modeCount = 0;
    for (const [bin, count] of bins) {
        if (count > modeCount) {
            modeBin = bin;
            modeCount = count;
        }
    }
    const mode = modeBin * binWidth + binWidth / 2.0;
    const modeFreq = modeCount;
    const count = clamped.length;
    const aMode = (modeFreq / count) * 100.0;
    const score = Math.min(1000.0, aMode / (2.0 * vr * (mode / 1000.0))) / 100.0;
    return Math.round(score * 100.0) / 100.0;
}
function spo2Points(samples) {
    const sorted = [...samples].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    const windowSize = 30;
    const step = 15;
    if (sorted.length < windowSize)
        return [];
    const output = [];
    for (let start = 0; start <= sorted.length - windowSize; start += step) {
        const window = sorted.slice(start, start + windowSize);
        const reds = window.map((s) => s.spo2Red);
        const irs = window.map((s) => s.spo2IR);
        const meanRed = (0, utils_1.average)(reds);
        const meanIR = (0, utils_1.average)(irs);
        if (meanRed < 1 || meanIR < 1)
            continue;
        const acRed = (0, utils_1.standardDeviation)(reds);
        const acIR = (0, utils_1.standardDeviation)(irs);
        if (acRed < 0.001 || acIR < 0.001)
            continue;
        const ratio = (acRed / meanRed) / (acIR / meanIR);
        const spo2 = (0, utils_1.clamp)(110.0 - 25.0 * ratio, 70, 100);
        const time = window[window.length - 1].timestamp;
        output.push({ timestamp: time, value: spo2 });
    }
    return output;
}
function skinTemperaturePoints(samples) {
    return samples
        .filter((s) => s.skinTempRaw >= 100)
        .map((s) => ({
        timestamp: s.timestamp,
        value: s.skinTempRaw * 0.04,
    }))
        .sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
}
function strainScore(samples, baseline) {
    const sorted = [...samples].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    if (sorted.length < 2)
        return null;
    const maxHR = 190.0;
    const resting = baseline.restingHeartRate > 0 ? baseline.restingHeartRate : 60.0;
    if (maxHR <= resting)
        return null;
    const hrReserve = maxHR - resting;
    const intervals = [];
    for (let i = 0; i < sorted.length - 1; i++) {
        intervals.push(Math.max(1.0, (sorted[i + 1].timestamp.getTime() - sorted[i].timestamp.getTime()) /
            1000));
    }
    const medianIntervalSeconds = intervals.length > 0
        ? [...intervals].sort((a, b) => a - b)[Math.floor(intervals.length / 2)]
        : 60.0;
    const fallbackIntervalMinutes = (0, utils_1.clamp)(medianIntervalSeconds / 60.0, 1.0 / 60.0, 5.0);
    let coveredMinutes = 0;
    let trimp = 0;
    for (let idx = 0; idx < sorted.length; idx++) {
        let dtMinutes;
        if (idx < sorted.length - 1) {
            const raw = (sorted[idx + 1].timestamp.getTime() -
                sorted[idx].timestamp.getTime()) /
                1000 /
                60;
            dtMinutes = (0, utils_1.clamp)(raw, 1.0 / 60.0, 5.0);
        }
        else {
            dtMinutes = fallbackIntervalMinutes;
        }
        coveredMinutes += dtMinutes;
        const pct = ((sorted[idx].heartRate - resting) / hrReserve) * 100.0;
        let weight;
        if (pct >= 90)
            weight = 5;
        else if (pct >= 80)
            weight = 4;
        else if (pct >= 70)
            weight = 3;
        else if (pct >= 60)
            weight = 2;
        else if (pct >= 50)
            weight = 1;
        else
            weight = 0;
        trimp += dtMinutes * weight;
    }
    if (coveredMinutes < 10)
        return null;
    if (trimp <= 0)
        return 0.0;
    const raw = (21.0 * Math.log(trimp + 1.0)) / STRAIN_LN_7201;
    const clamped = (0, utils_1.clamp)(raw, 0, 21);
    return Math.round(clamped * 100.0) / 100.0;
}
function sleepConsistencyScore(records, referenceDate) {
    const recent = [...records]
        .filter((record) => record.nightDate <= referenceDate)
        .sort((a, b) => a.nightDate.getTime() - b.nightDate.getTime())
        .slice(-7);
    if (recent.length < 3)
        return null;
    const durations = recent.map((r) => r.durationHours);
    const durationScore = Math.max(0, 100.0 - cv(durations));
    const startTimes = unwrapSleepTimes(recent.map((r) => secondsSinceMidnight(r.bedtime)));
    const endTimes = recent.map((r) => secondsSinceMidnight(r.wakeTime));
    const midpoints = unwrapSleepTimes(recent.map((r) => secondsSinceMidnight(new Date(r.bedtime.getTime() + (r.durationHours * 60 * 60 * 1000) / 2))));
    const timingScore = (0, utils_1.average)([
        Math.max(0, 100.0 - cv(startTimes)),
        Math.max(0, 100.0 - cv(endTimes)),
        Math.max(0, 100.0 - cv(midpoints)),
    ]);
    return (0, utils_1.clamp)((0, utils_1.average)([durationScore, timingScore]), 0, 100);
}
function sleepConsistencyScoreFromNightFeatures(records, referenceDate) {
    const recent = [...records]
        .filter((record) => record.nightDate <= referenceDate)
        .sort((a, b) => a.nightDate.getTime() - b.nightDate.getTime())
        .slice(-7);
    if (recent.length < 3)
        return null;
    const durations = recent.map((r) => r.sleepEstimateHours);
    const mean = (0, utils_1.average)(durations);
    if (mean <= 0)
        return null;
    const std = (0, utils_1.standardDeviation)(durations);
    const cvValue = (std / mean) * 100.0;
    return (0, utils_1.clamp)(100 - cvValue, 0, 100);
}
function secondsSinceMidnight(date) {
    return date.getHours() * 3600 + date.getMinutes() * 60 + date.getSeconds();
}
function unwrapSleepTimes(times) {
    return times.map((t) => (t > 64_800 ? t - 86_400 : t));
}
function cv(values) {
    if (values.length <= 1)
        return 0;
    const mean = (0, utils_1.average)(values);
    if (mean === 0)
        return 0;
    return ((0, utils_1.standardDeviation)(values) / Math.abs(mean)) * 100.0;
}
//# sourceMappingURL=derived-metrics.js.map