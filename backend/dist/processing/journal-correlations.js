...
exports.journalSleepCorrelations = journalSleepCorrelations;
const utils_1 = require("./utils");
function journalSleepCorrelations(journalEntries, sleepStageSummaries, sleepDetectionSummaries) {
    if (journalEntries.length === 0 || sleepStageSummaries.length < 3)
        return [];
    const stagesByDay = new Map();
    for (const s of sleepStageSummaries) {
        const key = (0, utils_1.dayKey)(s.nightDate);
        const arr = stagesByDay.get(key) ?? [];
        arr.push(s);
        stagesByDay.set(key, arr);
    }
    const detectionsByDay = new Map();
    for (const d of sleepDetectionSummaries) {
        const key = (0, utils_1.dayKey)(d.nightDate);
    ...
    if (allDeep.length === 0 || allDuration.length === 0)
        return [];
    const avgDeep = allDeep.reduce((a, b) => a + b, 0) / allDeep.length;
    const avgRem = allRem.reduce((a, b) => a + b, 0) / allRem.length;
    const avgDuration = allDuration.reduce((a, b) => a + b, 0) / allDuration.length;
    const byTag = new Map();
    ...
    for (const [tag, tagged] of byTag) {
        const taggedDays = new Set(tagged.map((e) => (0, utils_1.dayKey)(e.timestamp)));
        if (taggedDays.size < 2)
            continue;
        const tagDeep = [];
        const tagRem = [];
        const tagDuration = [];
        ...
        if (tagDeep.length === 0)
            continue;
        const tagAvgDeep = tagDeep.reduce((a, b) => a + b, 0) / tagDeep.length;
        const tagAvgRem = tagRem.reduce((a, b) => a + b, 0) / tagRem.length;
        const tagAvgDuration = tagDuration.length === 0
            ? 0
    ...
    return results.sort((a, b) => Math.abs(b.avgDeepDelta) +
        Math.abs(b.avgRemDelta) -
        (Math.abs(a.avgDeepDelta) + Math.abs(a.avgRemDelta)));
}
//# sourceMappingURL=journal-correlations.js.map