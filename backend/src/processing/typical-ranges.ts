...
export function computeTypicalRanges(
  detections: SleepDetectionSummary[],
  stages: SleepStageSummary[],
  excludeDate: Date,
): SleepTypicalRanges | null {
  const excludeKey = dayKey(excludeDate);

  const validDetections = detections.filter(
    (d) => dayKey(d.nightDate) !== excludeKey && d.confidence > 0.5,
  );
  const validStages = stages.filter((s) => {
    const total =
      ...
      s.unknownMinutes;
    return (
      dayKey(s.nightDate) !== excludeKey && s.confidence > 0.5 && total > 0
    );
  });

  if (validDetections.length < 3 || validStages.length < 3) return null;

  const durations = validDetections.map((d) => d.durationHours * 60);
  const avgDuration =
    durations.reduce((a, b) => a + b, 0) / durations.length;

  const restoratives = validStages.map(
    (s) => s.deepMinutes + s.remMinutes,
  );
  const avgRestorative =
    restoratives.reduce((a, b) => a + b, 0) / restoratives.length;

  function percentileRange(values: number[]): { lower: number; upper: number } {
    const sorted = [...values].sort((a, b) => a - b);
    const p25 = sorted[Math.max(0, Math.floor(sorted.length / 4))];
    const p75 =
      sorted[Math.min(sorted.length - 1, Math.floor((sorted.length * 3) / 4))];
