Pod::Spec.new do |s|
  s.name           = 'NoopCompute'
  s.version        = '1.0.0'
  s.summary        = 'On-device derived-metrics compute (noop Rust core via UniFFI)'
  s.description    = 'Expo module wrapping the noop compute-engine Rust core, cross-compiled for iOS and bridged with UniFFI.'
  s.author         = 'noop'
  s.homepage       = 'https://github.com/your-org/noop'
  s.license        = { :type => 'MIT' }
  s.platforms      = { :ios => '15.1' }
  s.swift_version  = '5.9'
  s.source         = { :git => 'https://github.com/noop/noop.git', :tag => "#{s.version}" }
  s.static_framework = true

  s.dependency 'ExpoModulesCore'

  # The Rust core ships as a static-library xcframework (device + sim slices).
  # CocoaPods' `vendored_frameworks` adds only a bare `-lnoop_compute_engine`
  ...
  s.preserve_paths = 'NoopComputeFFI.xcframework', 'noop_compute_engineFFI.h'
  s.user_target_xcconfig = {
    ...
    "NOOP_COMPUTE_SLICE[sdk=iphoneos*]" => "ios-arm64",
    "NOOP_COMPUTE_SLICE[sdk=iphonesimulator*]" => "ios-arm64-simulator",
    "OTHER_LDFLAGS" =>
      '$(inherited) -force_load "${PODS_ROOT}/../../modules/noop-compute/ios/NoopComputeFFI.xcframework/${NOOP_COMPUTE_SLICE}/libnoop_compute_engine.a"',
  }
  ...
  s.source_files = '*.swift', 'noop_compute_engineFFI.h'
  s.public_header_files = 'noop_compute_engineFFI.h'
end
