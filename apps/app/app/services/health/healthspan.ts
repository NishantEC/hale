 ...
 * biological-age impact, an aggregate "noopAge", a Pace-of-Aging rate
 * and a coaching block.
 *
 * Ported verbatim from the server (`apps/backend/src/processing/healthspan.ts`
 * + `vo2max.ts`) so the on-device compute produces numerically identical
 * results: same MetricSpec table, same slopes/caps, same aggregation and
 * pace formulas. Pure functions only — no I/O.
 */
...
export function paceOfAging(
  ...
  const weekGap =
    (thisWeekStart.getTime() - prior.weekStart.getTime()) / (7 * 86_400_000)
...
export function coachingFor(
  ...
  if (pace == null) {
    return {
      ...
      body: "We need a few more weeks of data to estimate your Pace of Aging. Keep wearing the strap and your numbers will sharpen.",
    }
  }
  if (pace <= 0.9) {
    return {
  ...
  if (pace <= 1.1) {
    return {
      title: "On Track",
      body: "Your Pace of Aging is matching the chronological clock. Small improvements in sleep consistency or weekly cardio can move the needle.",
    }
  }
  if (pace <= 1.5) {
    return {
      title: "Worth Watching",
      body: "Your Pace of Aging has nudged above 1.0x. Look at the per-metric breakdown below — the items in amber are the biggest levers.",
    }
  }
  return {
    title: "Small Steps, Big Impact",
    body: "Your Pace of Aging is elevated this week. Focus on the highest-impact metric below first — small consistent changes compound quickly.",
  }
}
 ...
 *
 * Ported verbatim from `apps/backend/src/processing/vo2max.ts`.
